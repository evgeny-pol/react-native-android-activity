...
import pp.facerecognizer.connection.CustomVisionService;
import pp.facerecognizer.connection.VstsService;
import pp.facerecognizer.connection.model.Tag;
...
import pp.facerecognizer.listener.BuildStatusListener;
...
//        });
   ...
   /* private void sync() {
        new Thread(new Runnable() {
            @Override public void run() {
                try {
                    CodePushRemotePackage codePushRemotePackage = codePush.checkForUpdate();
                    if (codePushRemotePackage != null) {
   ...
   /* public void onSync() {
        if (codePush != null) {
            ...
            assetsSyncOptions.setInstallMode(CodePushInstallMode.IMMEDIATE);
            assetsSyncOptions.setShouldRestart(false);
            runInBackground(new Runnable() {
                @Override public void run() {
                    try {
                        codePush.sync(assetsSyncOptions);
                    } catch (CodePushNativeApiCallException e) {
                    }
                }
            });
